    ...
    try {
        ...
        sponsors.forEach((sponsor, index) => {
            ...
            if (sponsorImageFile) {
                ...
                const sanitizedSponsorName = sponsor.name.replace(
                    /[^a-zA-Z0-9]/g,
                    "_"
                );
                ...
                sponsorImageUploadPromises.push(
                    (async () => {
                        try {
                            await file.save(
                                Buffer.from(
                                    await sponsorImageFile.arrayBuffer()
                                ),
                                {
                                    metadata: {
                                        contentType: sponsorImageFile.type,
                                    },
                                }
                            );
        ...
        if (userId) {
            try {
                console.log(
                    `Attempting to delete orphaned auth user: ${userId}`
                );
                await auth.deleteUser(userId);
                console.log(
                    `Successfully deleted orphaned auth user: ${userId}`
                );
            } catch (deleteError) {
                console.error(
    ...
    try {
        ...
        if (profileImage) {
            try {
                ...
                console.error(
                    "Error uploading volunteer profile image:",
                    uploadError
                );
            }
        } else {
        ...
        console.log(
            `Background Check Consent: ${consentString} -> ${hasConsent}`
        );
