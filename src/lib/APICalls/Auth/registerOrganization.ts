...
// FIX: Change return type to Promise<string | null>
const processProfileImage = async (
  ...
  // Now the return type matches the possible outcomes
  return profileImage
    ? await updateProfileImage(profileImage, userId, 'organizations')
    : null; // Return null explicitly if no image, aligning with updateProfileImage
  // Or return "" if you prefer an empty string for no image, the type still needs Promise<string | null>
  // Example using "":
  // const url = profileImage ? await updateProfileImage(profileImage, userId, "organizations") : "";
  // return url; // url is string | null if updateProfileImage returns null
};
...
const createOrganizationData = async (
  ...
  sponsors: SponsorData[],
  aidStock: AidStockDetails,
  profileImageUrl: string | null // <--- Accept string | null
  ...
  const organizationDataBase = {
    ...
    // FIX: Handle null by providing a default empty string
    profileImageUrl: profileImageUrl || '', // <--- Use empty string if null
    sponsors,
    aidStock,
