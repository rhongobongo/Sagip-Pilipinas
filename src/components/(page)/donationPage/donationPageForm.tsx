...
const DonationPageForm: React.FC<DonationPageFormProps> = ({
    ...
    const inventoryLimits = {
        ...
        searchAndRescue: {
            maxRescueKits:
                fetchedOrgData?.aidStock?.searchAndRescue?.rescueKits,
            maxRescuePersonnel:
    ...
    useEffect(() => {
        if (fetchedOrgData?.aidStock) {
            ...
            setCheckedDonationTypes({
                ...
                financialAssistance:
                    aidStock.financialAssistance?.available ?? false,
    ...
    const YOUR_PUBLIC_KEY =
        process.env.NEXT_PUBLIC_EMAILJS_PUBLIC_KEY || "RboVfwlHriKEa9DEr"; // Your key
    ...
    const handleDonationDetailChange = (
        ...
        if (event.target.type === "number") {
            ...
            // ... include all other else if conditions ...
                ...
                maxValue =
                    inventoryLimits.technicalSupport.maxCommunication ?? 0;
        ...
        setDonationDetails((prev) => {
            ...
            return {
                ...prev,
                [category]: { ...currentCategoryDetails, [field]: value },
            };
        });
    };
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        const checkedTypesArray = Object.entries(checkedDonationTypes).filter(
            ([, checked]) => checked
        );
        ...
        const checkedDonationTypesToSend: CheckedDonationTypes =
            Object.fromEntries(checkedTypesArray) as CheckedDonationTypes;
        ...
        for (const key in checkedDonationTypesToSend) {
            ...
            if (
                checkedDonationTypesToSend[donationTypeKey] &&
                ...
                switch (donationTypeKey) {
                    case "food":
                        ...
                        break;
                    case "clothing":
                        filteredDonationDetails.clothing =
                            donationDetails.clothing;
                        break;
                    case "medicalSupplies":
                        filteredDonationDetails.medicalSupplies =
                            donationDetails.medicalSupplies;
                        break;
                    case "shelter":
                        filteredDonationDetails.shelter =
                            donationDetails.shelter;
                        break;
                    case "searchAndRescue":
                        filteredDonationDetails.searchAndRescue =
                            donationDetails.searchAndRescue;
                        break;
                    case "financialAssistance":
                        filteredDonationDetails.financialAssistance =
                            donationDetails.financialAssistance;
                        break;
                    case "counseling":
                        filteredDonationDetails.counseling =
                            donationDetails.counseling;
                        break;
                    case "technicalSupport":
                        filteredDonationDetails.technicalSupport =
        ...
        try {
            ...
            if (!selectedPin) throw new Error("No selected pin");
            ...
            if (
                ...
                !donationResult.organizationId ||
                !donationResult.donationUID
            ) {
                throw new Error(
                    donationResult?.error ||
                        "Failed to submit donation or get necessary IDs."
                );
            }
            const orgId = donationResult.organizationId;
            const donationUID = donationResult.donationUID;
            console.log(
                `Donation successful. Org ID: ${orgId}, Donation UID: ${donationUID}`
            );
            setStatusMessage(
                "Donation recorded. Processing volunteer notifications..."
            );
            ...
            if (donationImage && donationUID) {
                try {
                    ...
                    console.warn(
                        "Donation saved, but image upload failed:",
                        imgError
                    );
                    setStatusMessage(
                        "Donation recorded, but image upload failed."
            ...
            try {
                ...
                if (!response.ok) {
                    ...
                    throw new Error(
                        `Failed to fetch volunteers: ${response.status} ${response.statusText} - ${errorData}`
                    );
                }
                volunteers = (await response.json()) as VolunteerData[];
                console.log(
                    `Workspaceed ${volunteers.length} volunteers with email.`
                );
            } catch (fetchError) {
                console.error("Could not fetch volunteers:", fetchError);
                setStatusMessage(
                    "Donation recorded, but could not fetch volunteers for notification."
                );
                submissionError =
            ...
            if (volunteers.length > 0) {
                ...
                if (!submissionError)
                    setStatusMessage(
                        `Donation recorded. Notifying ${volunteers.length} volunteers...`
                    );
                ...
                for (const volunteer of volunteers) {
                    if (!volunteer.email) {
                        console.warn(
                            `Skipping volunteer ${volunteer.userId} due to missing email.`
                        );
                        continue;
                    }
                    ...
                    console.log(
                        `DEBUG: Sending volunteer notification to ${volunteer.email} with params:`,
                    ...
                    try {
                        ...
                        console.log(
                            `EmailJS success sending to volunteer ${volunteer.email}`
                        );
                        emailSuccessCount++;
                    } catch (error: unknown) {
                        console.error(
                            `EmailJS failed sending to volunteer ${volunteer.email}:`,
                ...
                setStatusMessage(
                    "Donation processed successfully! No registered volunteers found for this organization."
                );
            }
    ...
    if (!fetchedOrgData) {
        return (
            <p className="text-center p-4">Loading organization details...</p>
        );
    }
        ...
        <form onSubmit={handleSubmit} className="h-full mx-4 mt-4">
            ...
            <div className="flex flex-col sm:flex-row gap-4 sm:gap-8 md:gap-16 lg:gap-32 justify-between sm:justify-evenly mt-4 mb-6 border-b pb-4">
                <div className="flex flex-col text-start gap-1 sm:gap-2">
                    <div className="flex">
                        <h2 className="font-semibold mr-1 text-sm">
                            Organization Name:
                        </h2>
                        <h2 className="text-sm">
                            {" "}
                            {fetchedOrgData.name ?? "N/A"}
                        </h2>
                    </div>
                    <div className="flex">
                        <h2 className="font-semibold mr-1 text-sm">
                ...
                <div className="flex flex-col text-start gap-1 sm:gap-2">
                    <div className="flex">
                        ...
                        <h2 className="text-sm">
                            {fetchedOrgData.contactNumber ?? "N/A"}
                        </h2>
                    </div>
                    <div className="flex">
                        <h2 className="font-semibold mr-1 text-sm">
                        ...
                        <h2 className="text-sm">
                            {fetchedOrgData.email ?? "N/A"}
                        </h2>
                    </div>
                </div>
            </div>
            {/* Main Container for Form Sections */}
            <div className="pinkContainerBorder space-y-4 mb-6">
                {" "}
                {/* Added mb-6 */}
                ...
                {selectedAidRequestId && (
                    <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-md">
                        <h3 className="text-md font-semibold text-red-700">
                            Donating towards Aid Request ID:{" "}
                            {selectedAidRequestId}
                        </h3>
                    </div>
                )}
                ...
                <div className="pinkBorder p-4">
                    <h1 className="text-lg font-semibold mb-3">
                        Purpose of Donation: (Select all applicable)
                    </h1>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            name="food.foodPacks"
                                            value={
                                                donationDetails.food?.foodPacks
                                            }
                                            onChange={
                                                handleDonationDetailChange
                                            }
                                            className="inputBox w-full"
                                            placeholder={`e.g., up to ${inventoryLimits.food.maxFoodPacks ?? "any"}`}
                                            min="0"
                                            max={
                                                inventoryLimits.food
                                                    .maxFoodPacks
                            ...
                            <div className="aid-detail-section">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                                    <div>
                                        <label className="block text-sm font-medium mb-1">
                                            Male: (Max:{" "}
                                            {inventoryLimits.clothing.maxMale ??
                                                "N/A"}
                                            )
                                        </label>
                                        <input
                                            ...
                                            name="clothing.male"
                                            value={
                                                donationDetails.clothing?.male
                                            }
                                            onChange={
                                                handleDonationDetailChange
                                            }
                                            className="inputBox w-full"
                                            placeholder={`e.g., up to ${inventoryLimits.clothing.maxMale ?? "any"}`}
                                            min="0"
                                            max={
                                                inventoryLimits.clothing.maxMale
                                            }
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            value={
                                                donationDetails.clothing?.female
                                            }
                                            onChange={
                                                handleDonationDetailChange
                                            }
                                            className="inputBox w-full"
                                            placeholder={`e.g., up to ${inventoryLimits.clothing.maxFemale ?? "any"}`}
                                            min="0"
                                            max={
                                                inventoryLimits.clothing
                                                    .maxFemale
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className="inputBox w-full"
                                            placeholder={`e.g., up to ${inventoryLimits.clothing.maxChildren ?? "any"}`}
                                            min="0"
                                            max={
                                                inventoryLimits.clothing
                                                    .maxChildren
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    <div>
                                        <label className="block text-sm font-medium mb-1">
                                            ...
                                            {inventoryLimits.medicalSupplies
                                                .maxMedicalKits ?? "N/A"}
                                            )
                                        </label>
                                        <input
                                            ...
                                            className="inputBox w-full"
                                            placeholder={`e.g., up to ${inventoryLimits.medicalSupplies.maxMedicalKits ?? "any"}`}
                                            min="0"
                                            max={
                                                inventoryLimits.medicalSupplies
                                                    .maxMedicalKits
                            ...
                            <div className="aid-detail-section">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    <div>
                                        <label className="block text-sm font-medium mb-1">
                                            Number of Tents: (Max:{" "}
                                            {inventoryLimits.shelter.maxTents ??
                                                "N/A"}
                                            )
                                        </label>
                                        <input
                                            ...
                                            name="shelter.tents"
                                            value={
                                                donationDetails.shelter?.tents
                                            }
                                            onChange={
                                                handleDonationDetailChange
                                            }
                                            className="inputBox w-full"
                                            placeholder={`e.g., up to ${inventoryLimits.shelter.maxTents ?? "any"}`}
                                            min="0"
                                            max={
                                                inventoryLimits.shelter.maxTents
                                            }
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className="inputBox w-full"
                                            placeholder={`e.g., up to ${inventoryLimits.shelter.maxBlankets ?? "any"}`}
                                            min="0"
                                            max={
                                                inventoryLimits.shelter
                                                    .maxBlankets
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    <div>
                                        <label className="block text-sm font-medium mb-1">
                                            ...
                                            {inventoryLimits.searchAndRescue
                                                .maxRescueKits ?? "N/A"}
                                            )
                                        </label>
                                        <input
                                            ...
                                            className="inputBox w-full"
                                            placeholder={`e.g., up to ${inventoryLimits.searchAndRescue.maxRescueKits ?? "any"}`}
                                            min="0"
                                            max={
                                                inventoryLimits.searchAndRescue
                                                    .maxRescueKits
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className="inputBox w-full"
                                            placeholder={`e.g., up to ${inventoryLimits.searchAndRescue.maxRescuePersonnel ?? "any"}`}
                                            min="0"
                                            max={
                                                inventoryLimits.searchAndRescue
                                                    .maxRescuePersonnel
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    <div>
                                        <label className="block text-sm font-medium mb-1">
                                            ...
                                            {inventoryLimits.financialAssistance.maxFunds?.toFixed(
                                                2
                                            ) ?? "N/A"}
                                            )
                                        </label>
                                        <input
                                            ...
                                            className="inputBox w-full"
                                            placeholder={`e.g., up to ${inventoryLimits.financialAssistance.maxFunds?.toFixed(2) ?? "any"}`}
                                            min="0"
                                            step="0.01"
                                            max={
                                                inventoryLimits
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className="inputBox w-full"
                                            placeholder={`e.g., up to ${inventoryLimits.counseling.maxCounselors ?? "any"}`}
                                            min="0"
                                            max={
                                                inventoryLimits.counseling
                                                    .maxCounselors
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className="inputBox w-full"
                                            placeholder={`e.g., up to ${inventoryLimits.counseling.maxHours ?? "any"}`}
                                            min="0"
                                            max={
                                                inventoryLimits.counseling
                                                    .maxHours
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    <div>
                                        <label className="block text-sm font-medium mb-1">
                                            ...
                                            {inventoryLimits.technicalSupport
                                                .maxVehicles ?? "N/A"}
                                            )
                                        </label>
                                        <input
                                            ...
                                            className="inputBox w-full"
                                            placeholder={`e.g., up to ${inventoryLimits.technicalSupport.maxVehicles ?? "any"}`}
                                            min="0"
                                            max={
                                                inventoryLimits.technicalSupport
                                                    .maxVehicles
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className="inputBox w-full"
                                            placeholder={`e.g., up to ${inventoryLimits.technicalSupport.maxCommunication ?? "any"}`}
                                            min="0"
                                            max={
                                                inventoryLimits.technicalSupport
                                                    .maxCommunication
                ...
                <div className="pinkBorder p-4">
                    <label
                        ...
                        className="text-lg font-semibold mb-3 block"
                    >
                        Estimated Drop-off / Volunteer Date:
                    </label>
                    <input
                        id="donation_date"
                        type="date"
            ...
            <div className="flex justify-center sm:justify-end pt-2 mb-4">
                <button
                    type="submit"
                    className="px-6 py-2 bg-[#B3002A] text-white rounded-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 disabled:opacity-50"
