...
const DonationPageForm: React.FC<DonationPageFormProps> = ({
    ...
    const handleDonationDetailChange = (
        ...
        if (event.target.type === "number") {
            ...
            if (category === "food" && field === "foodPacks")
                ...
                field === "rescuePersonnel"
            )
                maxValue =
                    inventoryLimits.searchAndRescue.maxRescuePersonnel ?? 0;
            else if (
                category === "financialAssistance" &&
                field === "totalFunds"
            )
            ...
            if (maxValue !== undefined) {
                const numValue =
                    field === "totalFunds"
                        ? parseFloat(value)
                        : parseInt(value); // Use parseFloat for funds
                if (!isNaN(numValue)) {
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        const checkedDonationTypesToSend: CheckedDonationTypes =
            Object.fromEntries(
                Object.entries(checkedDonationTypes).filter(
                    ([, isChecked]) => isChecked
                )
    ...
    if (!fetchedOrgData) {
        return (
            <p>Loading organization details or details are not available.</p>
        );
    }
                            ...
                            <div className="aid-detail-section">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium mb-1">
                                            Female: (Max:{" "}
                                            {inventoryLimits.clothing.maxFemale}
                                            )
                                        </label>
                                        <input
                                            type="number"
